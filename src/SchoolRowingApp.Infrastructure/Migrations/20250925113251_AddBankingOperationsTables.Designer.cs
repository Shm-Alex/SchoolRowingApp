// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using SchoolRowingApp.Infrastructure.Data;

#nullable disable

namespace SchoolRowingApp.Infrastructure.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20250925113251_AddBankingOperationsTables")]
    partial class AddBankingOperationsTables
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.19")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SchoolRowingApp.Domain.Athletes.Athlete", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SecondName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.ToTable("Athletes", (string)null);
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Athletes.AthletePayer", b =>
                {
                    b.Property<Guid>("AthleteId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("PayerId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("Id")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("PayerType")
                        .HasColumnType("integer");

                    b.HasKey("AthleteId", "PayerId");

                    b.HasIndex("PayerId");

                    b.ToTable("AthletePayers", (string)null);
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Banking.Transaction", b =>
                {
                    b.Property<DateTime>("OperationDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Currency")
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)");

                    b.Property<decimal>("BonusAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("CardLastDigits")
                        .HasMaxLength(4)
                        .HasColumnType("character varying(4)");

                    b.Property<decimal?>("Cashback")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<bool>("IsProcessed")
                        .HasColumnType("boolean");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("MccCode")
                        .HasMaxLength(4)
                        .HasColumnType("character varying(4)");

                    b.Property<decimal>("OperationAmountWithRoundUp")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("PaymentAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("PaymentCurrency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)");

                    b.Property<DateOnly>("PaymentDate")
                        .HasColumnType("date");

                    b.Property<decimal>("RoundUpAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("OperationDate", "Amount", "Currency");

                    b.HasIndex("Category");

                    b.HasIndex("IsProcessed");

                    b.HasIndex("PaymentDate");

                    b.HasIndex("Status");

                    b.ToTable("Transactions", "banking");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Banking.TransactionImport", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("ErrorCount")
                        .HasColumnType("integer");

                    b.Property<string>("FileHash")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ImportDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("SkippedCount")
                        .HasColumnType("integer");

                    b.Property<int>("SuccessCount")
                        .HasColumnType("integer");

                    b.Property<int>("TotalRows")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("TransactionImports");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Banking.TransactionImportDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ErrorMessage")
                        .HasColumnType("text");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("RawData")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Result")
                        .HasColumnType("integer");

                    b.Property<int>("RowNumber")
                        .HasColumnType("integer");

                    b.Property<Guid>("TransactionImportId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("TransactionImportId");

                    b.ToTable("TransactionImportDetails");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Membership.AthleteMembership", b =>
                {
                    b.Property<Guid>("AthleteId")
                        .HasColumnType("uuid");

                    b.Property<int>("MembershipPeriodYear")
                        .HasColumnType("integer");

                    b.Property<int>("MembershipPeriodMonth")
                        .HasColumnType("integer");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("ParticipationCoefficient")
                        .HasColumnType("decimal(3,1)");

                    b.HasKey("AthleteId", "MembershipPeriodYear", "MembershipPeriodMonth");

                    b.HasIndex("MembershipPeriodYear", "MembershipPeriodMonth");

                    b.ToTable("AthleteMemberships", "bob");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Membership.MembershipPeriod", b =>
                {
                    b.Property<int>("Year")
                        .HasColumnType("integer");

                    b.Property<int>("Month")
                        .HasColumnType("integer");

                    b.Property<decimal>("BaseFee")
                        .HasColumnType("decimal(10,2)");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Year", "Month");

                    b.ToTable("MembershipPeriods", "bob", t =>
                        {
                            t.HasCheckConstraint("CK_BaseFee", "BaseFee >= 0");

                            t.HasCheckConstraint("CK_Month", "Month >= 1 AND Month <= 12");

                            t.HasCheckConstraint("CK_Year", "Year >= 2020 AND Year <= 2100");
                        });
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Payments.Payer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Created")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SecondName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.ToTable("Payers", (string)null);
                });

            modelBuilder.Entity("TransactionTransactionImport", b =>
                {
                    b.Property<Guid>("TransactionImportsId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("TransactionsOperationDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("TransactionsAmount")
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("TransactionsCurrency")
                        .HasColumnType("character varying(3)");

                    b.HasKey("TransactionImportsId", "TransactionsOperationDate", "TransactionsAmount", "TransactionsCurrency");

                    b.HasIndex("TransactionsOperationDate", "TransactionsAmount", "TransactionsCurrency");

                    b.ToTable("TransactionTransactionImport");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Athletes.AthletePayer", b =>
                {
                    b.HasOne("SchoolRowingApp.Domain.Athletes.Athlete", "Athlete")
                        .WithMany("AthletePayers")
                        .HasForeignKey("AthleteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SchoolRowingApp.Domain.Payments.Payer", "Payer")
                        .WithMany("AthletePayers")
                        .HasForeignKey("PayerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Athlete");

                    b.Navigation("Payer");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Banking.TransactionImportDetail", b =>
                {
                    b.HasOne("SchoolRowingApp.Domain.Banking.TransactionImport", "TransactionImport")
                        .WithMany("ImportDetails")
                        .HasForeignKey("TransactionImportId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("TransactionImport");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Membership.AthleteMembership", b =>
                {
                    b.HasOne("SchoolRowingApp.Domain.Athletes.Athlete", "Athlete")
                        .WithMany("AthleteMemberships")
                        .HasForeignKey("AthleteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SchoolRowingApp.Domain.Membership.MembershipPeriod", "MembershipPeriod")
                        .WithMany("AthleteMemberships")
                        .HasForeignKey("MembershipPeriodYear", "MembershipPeriodMonth")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Athlete");

                    b.Navigation("MembershipPeriod");
                });

            modelBuilder.Entity("TransactionTransactionImport", b =>
                {
                    b.HasOne("SchoolRowingApp.Domain.Banking.TransactionImport", null)
                        .WithMany()
                        .HasForeignKey("TransactionImportsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SchoolRowingApp.Domain.Banking.Transaction", null)
                        .WithMany()
                        .HasForeignKey("TransactionsOperationDate", "TransactionsAmount", "TransactionsCurrency")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Athletes.Athlete", b =>
                {
                    b.Navigation("AthleteMemberships");

                    b.Navigation("AthletePayers");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Banking.TransactionImport", b =>
                {
                    b.Navigation("ImportDetails");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Membership.MembershipPeriod", b =>
                {
                    b.Navigation("AthleteMemberships");
                });

            modelBuilder.Entity("SchoolRowingApp.Domain.Payments.Payer", b =>
                {
                    b.Navigation("AthletePayers");
                });
#pragma warning restore 612, 618
        }
    }
}
